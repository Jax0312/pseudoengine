use crate::enums::{Array, Node, Position, VariableType};
                    ...
                    XFile {
                        handle: match mode.as_str() {
                            ...
                            "WRITE" => OpenOptions::new()
                                ...
                                .expect(""),
                            "READ" | "RANDOM" => {
                                let mut file = OpenOptions::new().write(true).read(true).create(true).append(false).open(filename.clone()).expect("");
...
pub fn run_close_file(executor: &mut Executor, filename: &Box<Node>) {
    ...
    if file.mode == "RANDOM" {
        ...
        file.handle.write_all(file.content.join("\n").as_bytes()).expect("");
    }
}
...
pub fn run_get_record(executor: &mut Executor, filename: &Box<Node>, destination: &Box<Node>) {
    let filename = run_expr(executor, filename).val_as_str();
    let dest_content = run_expr(executor, &Box::from(Node::Expression(vec![destination.clone()])));
    ...
    let mut data: Map<String, Value> = match file.content.get(file.cursor - 1) {
        ...
        None => runtime_err(format!("Line {} of file {} is empty", file.cursor, filename)),
    };


    if let Node::Object { name: _name, props: _props } = dest_content.deref() {
        let name = _name.clone();
        let mut props= _props.clone();
        deserialise_record(&mut props, &mut data);
        run_assign(executor, &destination, &Box::from(Node::Expression(vec![Box::from(Node::Object { name, props })])));
    }
...
fn deserialise_record(props: &mut HashMap<String, Property>, data: &mut Map<String, Value>) {
    props.iter_mut().for_each(|(k, property)| {
        match data.get_mut(k) {
            Some( dv) => {
                if let Property::Var { value, ref t, .. } = property {
                    *value = Box::from(
                        if let Node::Object { name, props } = value.deref_mut().deref_mut() {
                            // handle nested record type
                            if let Value::Object(map) = data.get_mut(name).unwrap() {
                                deserialise_record(props, map);
                                Node::Object { name: name.clone(), props: props.clone() }
                            } else {unreachable!()}

                        } else {
                            // convert Value to VariableType equivalent
                            let dv_type = match dv {
                                Value::Number(val) => if val.is_i64() { VariableType::Integer } else { VariableType::Real },
                                Value::String(val) => {
                            ...
                            value_to_node(value, dv, t.clone())
                        });
                }
            },
    ...
    props
        ...
        .map(|field| {
            if let Property::Var { value, t, private } = field.1 {
                match t.deref() {
                    VariableType::Custom(..) => {
                        if let Node::Object { props, .. } = value.deref() {
                            (field.0.clone(), Value::from(serialise_record(props)))
                        } else {
                            runtime_err("Invalid data type. RECORD data type expected".to_string())
                        }
                    },
                    _ => (field.0.clone(), Value::from(serialise_data(value)))
                }
            } else {
...
fn serialise_data(node: &Box<Node>) -> Value {
    match node.deref() {
        ...
        Node::Array {values, ..} => values.iter().map(|v| serialise_data(v)).collect(),
        _ => unreachable!(),
    }
}

fn value_to_node(old: &mut Box<Node>, new: &mut Value, new_type: Box<VariableType>) -> Node {
    let new_type = new_type.deref().clone(); 
    match new {
        Value::String(val) => if new_type == VariableType::String {
            Node::String { val: val.clone(), pos: Position::invalid() }
        } else {
            Node::Date { val: NaiveDate::parse_from_str(val.as_str(), "%Y-%m-%d").unwrap(), pos: Position::invalid() }
        },
        Value::Number(val) => if new_type == VariableType::Integer {
            Node::Int { val: val.as_i64().unwrap(), pos: Position::invalid() }
        } else {
            Node::Real { val: val.as_f64().unwrap(), pos: Position::invalid() }
        },
        Value::Bool(val) => Node::Boolean { val: val.clone(), pos: Position::invalid() },
        Value::Array(values) => {
            if let Node::Array { shape, .. } = old.clone().deref().deref(){
                if let VariableType::Array(arr_type) = new_type {
                    let mut len_of_narray = 0;
                    shape.iter().for_each(|index| len_of_narray += (index.upper - index.lower + 1) as usize);
                    if values.len() != len_of_narray {
                        runtime_err("Array shape mismatch while getting record".to_string())
                    }
                    Node::Array {
                        values: values.iter_mut().map(|v| Box::from(value_to_node(old, v, Box::from(arr_type.t.clone())))).collect(),
                        shape: shape.clone(),
                        pos: Position::invalid(),
                    }                    
                } else {
